...
WHERE Oficinas.Ciudad IN ('Navarra', 'Castell�n', 'Daimiel');
...
--8
SELECT COUNT (Representantes.Oficina_Rep) as [N�mero de Representantes], Oficinas.Region
FROM Representantes
INNER JOIN Oficinas
ON Oficinas.Oficina = Representantes.Oficina_Rep
...
--10
SELECT Clientes.Empresa, Representantes.Nombre, Oficinas.Ciudad, SUM (Pedidos.Cantidad) as [N�mero de Pedidos]
FROM Clientes
INNER JOIN Pedidos
ON Clientes.Num_Cli = Pedidos.Cliente
INNER JOIN Representantes
